...
public class MonsterImageCache {
    ...
    // Methods
    static BufferedImageIcon getCachedImage(final String name, final int level, final Element e) {
	if (!MonsterImageCache.isInCache(name)) {
	    BufferedImageIcon bii = MonsterImageManager.getUncachedImage(name, level);
	    bii = ImageTransformer.templateTransformMonsterImage(bii, e, MonsterImageManager.MONSTER_IMAGE_SIZE);
