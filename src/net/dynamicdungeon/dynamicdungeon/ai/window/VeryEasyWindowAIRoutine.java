package net.dynamicdungeon.dynamicdungeon.ai.window;
...
public class VeryEasyWindowAIRoutine extends AbstractWindowAIRoutine {
	...
	} else if (CommonWindowAIRoutines.check(VeryEasyWindowAIRoutine.STEAL_CHANCE)) {
	    // Steal
	    return AbstractWindowAIRoutine.ACTION_STEAL;
	} else if (CommonWindowAIRoutines.check(VeryEasyWindowAIRoutine.DRAIN_CHANCE)) {
	    // Drain MP
	    return AbstractWindowAIRoutine.ACTION_DRAIN;
	} else if (CommonWindowAIRoutines.check(VeryEasyWindowAIRoutine.FLEE_CHANCE)) {
	    // Flee
	...
	if (chance <= VeryEasyWindowAIRoutine.CAST_SPELL_CHANCE) {
		...
		if (randomSpellID == CommonWindowAIRoutines.SPELL_INDEX_HEAL) {
		    ...
		    if (c.getCurrentHP() > c.getMaximumHP() * VeryEasyWindowAIRoutine.HEAL_THRESHOLD / 100) {
			// Do not need healing
			return false;
		    }
		}
