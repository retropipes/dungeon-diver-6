...
public class Shop {
	...
	private boolean shopStage2() {
		...
		Shop.this.choices = EquipmentFactory.createWeaponNames(playerCharacter.getCaste().getCasteID());
	    } else if (Shop.this.type == ShopTypes.SHOP_TYPE_ARMOR) {
		Shop.this.choices = EquipmentFactory.createArmorNames(Shop.this.typeIndex);
	...
	private boolean shopStage3() {
		...
		return false;
	    } else if (Shop.this.type == ShopTypes.SHOP_TYPE_SPELLS && playerCharacter.getSpellBook()
	    ...
	    Shop.this.result = CommonDialogs.showInputDialog(Shop.this.getGoldTotals() + "Select",
		    Shop.this.getShopNameFromType(), Shop.this.choices, Shop.this.choices[Shop.this.defaultChoice]);
	...
	private boolean shopStage4() {
	    ...
	    final PartyMember playerCharacter = PartyManager.getParty().getLeader();
	    Shop.this.cost = 0;
	    if (Shop.this.type == ShopTypes.SHOP_TYPE_WEAPONS || Shop.this.type == ShopTypes.SHOP_TYPE_ARMOR) {
		...
		while (stage4Result.equals("")) {
		    stage4Result = CommonDialogs.showTextInputDialog(
			    Shop.this.getGoldTotals() + Shop.this.result + " How Much?",
	...
	private void shopStage6() {
		...
		final Equipment bought = EquipmentFactory.createWeapon(Shop.this.index,
			playerCharacter.getCaste().getCasteID(), 0);
		playerCharacter.getItems().equipWeapon(playerCharacter, bought, true);
	    } else if (Shop.this.type == ShopTypes.SHOP_TYPE_ARMOR) {
		playerCharacter.offsetGold(-Shop.this.cost);
		final Equipment bought = EquipmentFactory.createArmor(Shop.this.index, Shop.this.typeIndex, 0);
