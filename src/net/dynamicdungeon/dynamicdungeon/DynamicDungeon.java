...
public class DynamicDungeon {
    ...
    private static final ErrorLogger elog = new ErrorLogger(DynamicDungeon.PROGRAM_NAME);
	...
	CommonDialogs.showErrorDialog(DynamicDungeon.ERROR_MESSAGE, DynamicDungeon.ERROR_TITLE);
	return DynamicDungeon.elog;
    }

    public static void preInit() {
	// Compute action cap
	AbstractCreature.computeActionCap(DynamicDungeon.BATTLE_MAZE_SIZE, DynamicDungeon.BATTLE_MAZE_SIZE);
	// HiDPI detection
	...
	try {
	    ...
	    Platform.hookAbout(DynamicDungeon.application.getAboutDialog(),
		    DynamicDungeon.application.getAboutDialog().getClass().getDeclaredMethod("showAboutDialog"));
	    ...
	    Platform.hookQuit(DynamicDungeon.application.getGUIManager(),
		    DynamicDungeon.application.getGUIManager().getClass().getDeclaredMethod("quitHandler"));
