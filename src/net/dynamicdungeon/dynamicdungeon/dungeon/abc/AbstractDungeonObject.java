...
public abstract class AbstractDungeonObject extends CloneableObject implements RandomGenerationRule {
     ...
     * @return
     */
    public AbstractDungeonObject gameRenderHook(final int x, final int y, final int z) {
	return this;
    }
    ...
    @Override
    public boolean shouldGenerateObject(final Dungeon maze, final int row, final int col, final int floor,
    ...
    public final void writeDungeonObject(final DatabaseWriter writer) throws IOException {
    ...
    public final AbstractDungeonObject readDungeonObjectV1(final DatabaseReader reader, final String ident)
	    throws IOException {
	if (ident.equals(this.getIdentifier())) {
	    final String savedIdent = reader.readString();
	    if (!savedIdent.equals("NULL")) {
		this.saved = DynamicDungeon.getApplication().getObjects().readSavedDungeonObject(reader, savedIdent,
			FormatConstants.MAZE_FORMAT_LATEST);
	    }
	    final int cc = this.getCustomFormat();
	    if (cc == AbstractDungeonObject.CUSTOM_FORMAT_MANUAL_OVERRIDE) {
		return this.readDungeonObjectHook(reader, FormatConstants.MAZE_FORMAT_LATEST);
	    } else {
		for (int x = 0; x < cc; x++) {
     ...
     * @throws IOException
     */
    protected void writeDungeonObjectHook(final DatabaseWriter writer) throws IOException {
     ...
     * @throws IOException
     */
    protected AbstractDungeonObject readDungeonObjectHook(final DatabaseReader reader, final int formatVersion)
	    throws IOException {
