...
public class GenerateTask extends Thread {
	...
	try {
	    ...
	    final int mazeSize = DungeonConstants.MAZE_SIZE_BASE + levelCount * DungeonConstants.MAZE_SIZE_INCREMENT;
	    ...
	    final RandomRange rF = new RandomRange(0, Dungeon.getMaxFloors() - 1);
	    if (this.scratch) {
		...
		do {
		    ...
		    startC = rC.generate();
		    startF = rF.generate();
		} while (gameDungeon.getCell(startR, startC, startF, DungeonConstants.LAYER_OBJECT).isSolid());
	    ...
	    AbstractDungeonObject
		    .setTemplateColor(ImageColorConstants.getColorForLevel(PartyManager.getParty().getDungeonLevel()));
	    if (this.scratch) {
