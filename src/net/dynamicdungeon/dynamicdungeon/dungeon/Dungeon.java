...
public class Dungeon {
	...
	this.basePath = DynamicProperties.getCachesDirectory() + File.separator + "DynamicDungeonTemp" + File.separator
		+ randomID + ".maze";
	final File base = new File(this.basePath);
	final boolean success = base.mkdirs();
	if (!success) {
	    CommonDialogs.showErrorDialog("Dungeon temporary folder creation failed!", "DynamicDungeon");
	}
    }

    // Static methods
    public static String getDungeonTempFolder() {
	return DynamicProperties.getCachesDirectory() + File.separator + "DynamicDungeonTemp";
    }
	...
	try (DatabaseReader metaReader = new DatabaseReader(m.basePath + File.separator + "metafile.xml")) {
	    // Read metafile
	...
	return new DatabaseReader(this.basePath + File.separator + "level" + this.activeLevel + ".xml");
    }

    private int readDungeonMetafile(final DatabaseReader reader) throws IOException {
    ...
    private void readDungeonLevel(final DatabaseReader reader) throws IOException {
	this.readDungeonLevel(reader, FormatConstants.MAZE_FORMAT_LATEST);
    }

    private void readDungeonLevel(final DatabaseReader reader, final int formatVersion) throws IOException {
	...
	try {
	    ...
	    try (DatabaseWriter metaWriter = new DatabaseWriter(this.basePath + File.separator + "metafile.xml")) {
		// Write metafile
	...
	return new DatabaseWriter(this.basePath + File.separator + "level" + this.activeLevel + ".xml");
    }

    private void writeDungeonMetafile(final DatabaseWriter writer) throws IOException {
	if (this.prefixHandler != null) {
    ...
    private void writeDungeonLevel(final DatabaseWriter writer) throws IOException {
	// Write the level
