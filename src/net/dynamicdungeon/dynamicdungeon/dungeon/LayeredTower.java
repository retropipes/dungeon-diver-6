...
final class LayeredTower implements Cloneable {
	...
	this.data = new LowLevelADODataStore(cols, rows, floors, DungeonConstants.LAYER_COUNT);
	this.savedDungeonState = new LowLevelADODataStore(cols, rows, floors, DungeonConstants.LAYER_COUNT);
	...
	try {
	    final AbstractDungeonObject there = this.getCell(xLoc + dirMove[0], yLoc + dirMove[1], zLoc,
		    DungeonConstants.LAYER_OBJECT);
	    final AbstractDungeonObject ground = this.getCell(xLoc + dirMove[0], yLoc + dirMove[1], zLoc,
		...
		if (LayeredTower.radialScan(xLoc, yLoc, 0, pLocX, pLocY)) {
		    ...
		    // Move the monster
		    this.setCell(monster.getSavedObject(), xLoc, yLoc, zLoc, DungeonConstants.LAYER_OBJECT);
		    monster.setSavedObject(there);
		    this.setCell(monster, xLoc + dirMove[0], yLoc + dirMove[1], zLoc, DungeonConstants.LAYER_OBJECT);
	...
	final int zLoc = this.getPlayerFloor();
	if (!player) {
	    this.setCell(saved, xLoc, yLoc, zLoc, DungeonConstants.LAYER_OBJECT);
	}
	...
	AbstractDungeonObject currObj = this.getCell(randomRow, randomColumn, zLoc, DungeonConstants.LAYER_OBJECT);
	if (!currObj.isSolid()) {
	    final Monster m = new Monster();
	    m.setSavedObject(currObj);
	    this.setCell(m, randomRow, randomColumn, zLoc, DungeonConstants.LAYER_OBJECT);
	} else {
	    while (currObj.isSolid()) {
		randomRow = row.generate();
		randomColumn = column.generate();
		currObj = this.getCell(randomRow, randomColumn, zLoc, DungeonConstants.LAYER_OBJECT);
	    }
	    final Monster m = new Monster();
	    m.setSavedObject(currObj);
	    this.setCell(m, randomRow, randomColumn, zLoc, DungeonConstants.LAYER_OBJECT);
    ...
    public AbstractDungeonObject getCell(final int row, final int col, final int floor, final int extra) {
	int fR = row;
	int fC = col;
	final int fF = floor;
	...
	do {
	    ...
	    try {
		final AbstractDungeonObject obj = this.getCell(fx1, fy1, zLoc, DungeonConstants.LAYER_OBJECT);
    ...
    public void fillFloor(final AbstractDungeonObject bottom, final AbstractDungeonObject top, final int z) {
	int x, y, e;
    ...
    public void fillFloorRandomly(final Dungeon maze, final int z, final int w) {
	// Pre-Pass
	final DungeonObjectList objects = DynamicDungeon.getApplication().getObjects();
	...
	for (int layer = 0; layer < DungeonConstants.LAYER_COUNT; layer++) {
	    final AbstractDungeonObject[] requiredObjects = objects.getAllRequired(layer);
	...
	for (x = 0; x < this.getColumns(); x++) {
		...
		for (z = 0; z < this.getFloors(); z++) {
		    for (e = 0; e < DungeonConstants.LAYER_COUNT; e++) {
			this.savedDungeonState.setCell(this.getCell(y, x, z, e), x, y, z, e);
		    }
		}
	    }
	}
	...
	for (x = 0; x < this.getColumns(); x++) {
		...
		for (z = 0; z < this.getFloors(); z++) {
		    for (e = 0; e < DungeonConstants.LAYER_COUNT; e++) {
			this.setCell(this.savedDungeonState.getCell(x, y, z, e), y, x, z, e);
		    }
		}
	    }
	}
	...
	for (x = 0; x < this.getColumns(); x++) {
	    for (y = 0; y < this.getRows(); y++) {
		final AbstractDungeonObject mo = this.getCell(y, x, floor, DungeonConstants.LAYER_OBJECT);
		if (mo != null) {
		    mo.tickTimer(y, x);
		}
	    }
    ...
    public void writeLayeredDungeon(final DatabaseWriter writer) throws IOException {
	int y, x, z, e;
    ...
    public static LayeredTower readLayeredDungeonV1(final DatabaseReader reader) throws IOException {
	...
	for (x = 0; x < lt.getColumns(); x++) {
		...
		for (z = 0; z < lt.getFloors(); z++) {
		    for (e = 0; e < DungeonConstants.LAYER_COUNT; e++) {
			lt.setCell(DynamicDungeon.getApplication().getObjects().readDungeonObject(reader,
    ...
    public void writeSavedDungeonState(final DatabaseWriter writer) throws IOException {
	int x, y, z, e;
	writer.writeInt(this.getColumns());
	writer.writeInt(this.getRows());
	writer.writeInt(this.getFloors());
	for (x = 0; x < this.getColumns(); x++) {
		...
		for (z = 0; z < this.getFloors(); z++) {
		    for (e = 0; e < DungeonConstants.LAYER_COUNT; e++) {
			this.savedDungeonState.getCell(y, x, z, e).writeDungeonObject(writer);
		    }
		}
	    }
	}
    ...
    public void readSavedDungeonState(final DatabaseReader reader, final int formatVersion) throws IOException {
	int x, y, z, e, sizeX, sizeY, sizeZ;
	sizeX = reader.readInt();
	sizeY = reader.readInt();
	sizeZ = reader.readInt();
	this.savedDungeonState = new LowLevelADODataStore(sizeY, sizeX, sizeZ, DungeonConstants.LAYER_COUNT);
	for (x = 0; x < sizeY; x++) {
	    for (y = 0; y < sizeX; y++) {
		for (z = 0; z < sizeZ; z++) {
		    for (e = 0; e < DungeonConstants.LAYER_COUNT; e++) {
			this.savedDungeonState.setCell(
				DynamicDungeon.getApplication().getObjects().readDungeonObject(reader, formatVersion),
				y, x, z, e);
		    }
		}
	    }
