...
public class Monster extends AbstractMovingObject {
	...
	if (DynamicDungeon.getApplication().getMode() != Application.STATUS_BATTLE) {
	    DynamicDungeon.getApplication().getBattle().doBattle();
	    DynamicDungeon.getApplication().getDungeonManager().getDungeon().postBattle(this, dirX, dirY, true);
	}
    }
	...
	DynamicDungeon.getApplication().getDungeonManager().getDungeon().updateMonsterPosition(move, dirX, dirY, this);
	this.activateTimer(1);
    }
