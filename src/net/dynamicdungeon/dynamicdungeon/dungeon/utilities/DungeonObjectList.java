...
public class DungeonObjectList {
	...
	final AbstractDungeonObject[] allObjects = { new ArmorShop(), new Bank(), new ClockwiseRotationTrap(),
		new ClosedDoor(), new ConfusionTrap(), new CounterclockwiseRotationTrap(), new DarkGem(),
		new DizzinessTrap(), new DrunkTrap(), new Empty(), new EmptyVoid(), new HealShop(), new HealTrap(),
		new HurtTrap(), new Ice(), new ItemShop(), new LightGem(), new Monster(), new OpenDoor(),
		new Regenerator(), new SealingWall(), new SpellShop(), new Tile(), new UTurnTrap(),
		new VariableHealTrap(), new VariableHurtTrap(), new Wall(), new WarpTrap(), new WeaponsShop(),
		new StairsUp(), new StairsDown(), new WallOff(), new WallOn(), new Button(), new Amulet(), new Stone(),
		new Bomb(), new Hammer(), new Exit(), new Key(), new Tablet(), new Lock(), new TabletSlot() };
	...
	for (int x = 0; x < allEditorAppearances.length; x++) {
	    allEditorAppearances[x] = ImageTransformer.getTransformedImage(ObjectImageManager
		    .getImage(objects[x].getName(), objects[x].getBaseID(), AbstractDungeonObject.getTemplateColor()),
    ...
    public final AbstractDungeonObject[] getAllWithoutPrerequisiteAndNotRequired(final int layer) {
	final AbstractDungeonObject[] objects = this.getAllObjects();
    ...
    public AbstractDungeonObject readDungeonObject(final DatabaseReader reader, final int formatVersion)
	    throws IOException {
	final AbstractDungeonObject[] objects = this.getAllObjects();
    ...
    public AbstractDungeonObject readSavedDungeonObject(final DatabaseReader reader, final String UID,
