...
public class DungeonEffectManager {
	...
	this.activeEffects[DungeonEffectConstants.EFFECT_ROTATED_CLOCKWISE] = new RotatedCW(0);
	this.activeEffects[DungeonEffectConstants.EFFECT_ROTATED_COUNTERCLOCKWISE] = new RotatedCCW(0);
	this.activeEffects[DungeonEffectConstants.EFFECT_U_TURNED] = new UTurned(0);
	this.activeEffects[DungeonEffectConstants.EFFECT_CONFUSED] = new Confused(0);
	...
	this.activeEffects[DungeonEffectConstants.EFFECT_STICKY] = new Sticky(0);
	this.activeEffects[DungeonEffectConstants.EFFECT_POWER_GATHER] = new PowerGather(0);
	this.activeEffects[DungeonEffectConstants.EFFECT_POWER_WITHER] = new PowerWither(0);
	// Create GUI
	...
	this.activeEffectMessageContainer.setLayout(new GridLayout(DungeonEffectManager.MAX_ACTIVE_EFFECTS, 1));
	...
	for (int x = 0; x < DungeonEffectManager.NUM_EFFECTS; x++) {
		...
		if (!this.activeEffects[x].isActive()) {
		    DynamicDungeon.getApplication().showMessage("You feel normal again.");
		    // Clear effect grid
		    this.clearGridEntry(x);
		    // Pack
		    DynamicDungeon.getApplication().getGameManager().getOutputFrame().pack();
		}
	    }
	}
    }
	...
	this.activateEffectInternal(effectID,
		DungeonEffectConstants.DURATIONS[PreferencesManager.getGameDifficulty()][effectID]);
    }
