...
public final class DungeonManager {
    ...
    public AbstractDungeonObject getDungeonObject(final int x, final int y, final int z, final int e) {
	try {
	...
	while (!FilenameChecker.isFilenameOK(returnVal)) {
		...
		if (!FilenameChecker.isFilenameOK(returnVal)) {
			...
			if (!okay) {
			    DynamicDungeon.getErrorLogger().logError(new IOException("Cannot create game folder!"));
			}
		    }
	...
	DynamicDungeon.getApplication().showMessage("Saving " + sg + " file...");
