package net.dynamicdungeon.dynamicdungeon.battle.window.turn;
...
import net.dynamicdungeon.dynamicdungeon.battle.BattleResults;
import net.dynamicdungeon.dynamicdungeon.battle.damageengines.AbstractDamageEngine;
...
import net.dynamicdungeon.dynamicdungeon.creatures.monsters.AbstractMonster;
import net.dynamicdungeon.dynamicdungeon.creatures.monsters.BossMonster;
import net.dynamicdungeon.dynamicdungeon.creatures.monsters.MonsterFactory;
...
import net.dynamicdungeon.dynamicdungeon.items.combat.CombatItemChucker;
...
import net.dynamicdungeon.dynamicdungeon.spells.SpellCaster;
...
public class WindowTurnBattleLogic extends AbstractBattle {
	...
	try {
	    ...
	    if (app.getDungeonManager().getDungeon().getBombs() > 0) {
		// Use Bomb
		app.getDungeonManager().getDungeon().useBomb();
		SoundManager.playSound(SoundConstants.SOUND_EXPLODE);
		app.showMessage("Your bomb explodes, destroying the monster!");
		this.doBattleByProxy();
	    } else {
	...
	if (playerCharacter.checkLevelUp()) {
	    ...
	    DynamicDungeon.getApplication().showMessage("You reached level " + playerCharacter.getLevel() + ".");
	}
    }
    ...
    @Override
    public void redrawOneBattleSquare(final int x, final int y, final AbstractDungeonObject obj3) {
	// Do nothing
    }
