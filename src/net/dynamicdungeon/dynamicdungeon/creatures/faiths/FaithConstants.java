...
public class FaithConstants {
	...
	if (!FaithConstants.INITED) {
		...
		try (final ResourceStreamReader rsr1 = new ResourceStreamReader(FaithConstants.class
		...
		FaithConstants.FAITH_NAMES = tempNames.toArray(new String[tempNames.size()]);
		FaithConstants.FAITHS_COUNT = FaithConstants.FAITH_NAMES.length;
		final ArrayList<String> tempColors = new ArrayList<>();
		try (final ResourceStreamReader rsr2 = new ResourceStreamReader(FaithConstants.class
		...
		final String[] tempColors2 = tempColors.toArray(new String[tempColors.size()]);
		...
		for (int x = 0; x < tempColors2.length; x++) {
		    ...
		    FaithConstants.FAITH_COLORS[x] = new Color(tempColors3[x][0], tempColors3[x][1], tempColors3[x][2]);
		}
		if (FaithConstants.FAITH_DISPLAY_NAMES == null) {
		    final String[] temp = new String[FaithConstants.FAITHS_COUNT];
		    for (int x = 0; x < temp.length; x++) {
			temp[x] = NamesManager.getName(NamesConstants.SECTION_FAITHS,
