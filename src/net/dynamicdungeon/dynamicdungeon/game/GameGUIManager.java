...
import net.dynamicdungeon.dynamicdungeon.DrawGrid;
...
import net.dynamicdungeon.dynamicdungeon.dungeon.objects.Darkness;
import net.dynamicdungeon.dynamicdungeon.dungeon.objects.DungeonNoteObject;
...
class GameGUIManager {
	...
	if (this.vwMgr == null) {
	    this.vwMgr = DynamicDungeon.getApplication().getGameManager().getViewManager();
	    this.setUpGUI();
	}
    }
		...
		for (y = this.vwMgr.getViewingWindowLocationY(); y <= this.vwMgr
			...
			if (visible) {
			    final AbstractDungeonObject obj1 = m
				    .getCell(y, x, m.getPlayerLocationZ(), DungeonConstants.LAYER_GROUND)
				    .gameRenderHook(y, x, m.getPlayerLocationZ());
			    final AbstractDungeonObject obj2 = m
				    .getCell(y, x, m.getPlayerLocationZ(), DungeonConstants.LAYER_OBJECT)
				    .gameRenderHook(y, x, m.getPlayerLocationZ());
			    final BufferedImageIcon img1 = ObjectImageManager.getImage(obj1.getName(),
				    obj1.getGameBaseID(), AbstractDungeonObject.getTemplateColor());
			    final BufferedImageIcon img2 = ObjectImageManager.getImage(obj2.getName(),
				    obj2.getGameBaseID(), AbstractDungeonObject.getTemplateColor());
			    if (u == y && v == x) {
				final AbstractDungeonObject obj3 = new Player().gameRenderHook(y, x,
					m.getPlayerLocationZ());
				final BufferedImageIcon img3 = ObjectImageManager.getImage(obj3.getName(),
					obj3.getGameBaseID(), AbstractDungeonObject.getTemplateColor());
				...
				final AbstractDungeonObject obj3 = GameGUIManager.NOTE.gameRenderHook(y, x,
					m.getPlayerLocationZ());
				final BufferedImageIcon img3 = ObjectImageManager.getImage(obj3.getName(),
					obj3.getGameBaseID(), AbstractDungeonObject.getTemplateColor());
				this.drawGrid.setImageCell(ImageTransformer.getVirtualCompositeImage(img1, img2, img3,
			...
			final AbstractDungeonObject obj = ev.gameRenderHook(y, x, m.getPlayerLocationZ());
			this.drawGrid.setImageCell(ObjectImageManager.getImage(obj.getName(), obj.getGameBaseID(),
				AbstractDungeonObject.getTemplateColor()), xFix, yFix);
	...
	this.outputFrame = new JFrame("DynamicDungeon");
	...
	public void handleMovement(final KeyEvent e) {
	    try {
		final GameLogicManager glm = DynamicDungeon.getApplication().getGameManager();
	...
	public void mouseClicked(final MouseEvent e) {
	    try {
		final GameLogicManager gm = DynamicDungeon.getApplication().getGameManager();
		if (e.isShiftDown()) {
		    final int x = e.getX();
		    final int y = e.getY();
