...
public final class GameLogicManager {
	...
	final int curr = DynamicDungeon.getApplication().getDungeonManager().getDungeon().getStones() * 100;
	final int max = this.stoneCount;
	return "Power: " + curr / max + "%";
    }

    public void updateStoneCount() {
	this.stoneCount = DynamicDungeon.getApplication().getDungeonManager().getDungeon().getStoneCount();
    }
	...
	try {
	    final AbstractDungeonObject target1 = m.getCell(destX, destY, destZ, DungeonConstants.LAYER_GROUND);
	    final AbstractDungeonObject target2 = m.getCell(destX, destY, destZ, DungeonConstants.LAYER_OBJECT);
	    ...
	    DynamicDungeon.getApplication().showMessage(gameName2 + " on " + gameName1);
