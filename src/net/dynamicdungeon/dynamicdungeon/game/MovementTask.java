...
final class MovementTask extends Thread {
    ...
    // Constructors
    public MovementTask(final GameViewingWindowManager view, final DungeonEffectManager effect,
	...
	try {
	    ...
	    final AbstractDungeonObject below = m.getCell(m.getPlayerLocationX(), m.getPlayerLocationY(),
		    m.getPlayerLocationZ(), DungeonConstants.LAYER_GROUND);
	    final AbstractDungeonObject nextBelow = m.getCell(x, y, z, DungeonConstants.LAYER_GROUND);
	    final AbstractDungeonObject nextAbove = m.getCell(x, y, z, DungeonConstants.LAYER_OBJECT);
	    return MovementTask.checkSolidAbsolute(this.saved, below, nextBelow, nextAbove);
	...
	final Dungeon m = DynamicDungeon.getApplication().getDungeonManager().getDungeon();
    ...
    private static boolean checkSolidAbsolute(final AbstractDungeonObject inside, final AbstractDungeonObject below,
	...
	do {
		...
		try {
		    nextBelow = m.getCell(px + fX, py + fY, pz + fZ, DungeonConstants.LAYER_GROUND);
		} catch (final ArrayIndexOutOfBoundsException ae) {
		    nextBelow = new Empty();
		}
		try {
		    nextAbove = m.getCell(px + fX, py + fY, pz + fZ, DungeonConstants.LAYER_OBJECT);
		} catch (final ArrayIndexOutOfBoundsException ae) {
		    nextAbove = new Wall();
		}
		try {
		...
		try {
			...
			if (this.proceed) {
			    this.saved = m.getCell(px, py, pz, DungeonConstants.LAYER_OBJECT);
	...
	return this.proceed && !this.em.isEffectActive(DungeonEffectConstants.EFFECT_STICKY) && !nextBelow.hasFriction()
	...
	final Dungeon m = app.getDungeonManager().getDungeon();
	try {
	    m.getCell(x, y, z, DungeonConstants.LAYER_OBJECT).preMoveAction(true, x, y);
	} catch (final ArrayIndexOutOfBoundsException ae) {
	    // Ignore
	} catch (final NullPointerException np) {
	    // Ignore
	...
	try {
		...
		this.saved = m.getCell(m.getPlayerLocationX(), m.getPlayerLocationY(), m.getPlayerLocationZ(),
